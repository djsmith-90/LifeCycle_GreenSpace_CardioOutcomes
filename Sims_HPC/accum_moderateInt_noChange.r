...
### This script is for: BMI caused by accumulation and interaction with this and SEP (interaction size = moderate [0.67 for binary exposures; 0.02 for continuous exposures])
...
lasso_sim <- function(n_sims = 1000, sampleSize = 1000, Exposure = "Binary", Centered = "No", Collinear = "Low",
  ...
  for (i in 1:n_sims) {
	...
	if (Exposure == "Binary") {
		bmi <- 25 + (-4 * high_sep) + (-0.67 * accumulation) + (0.67 * high_sep * accumulation) + rnorm(n, 0, 3)
	}
	
	if (Exposure == "Cont") {
		bmi <- 28 + (-4 * high_sep) + (-0.02 * accumulation) + (0.02 * high_sep * accumulation) + rnorm(n, 0, 3)
	}
	if (Output == TRUE) {
    ...
    AIC_res_temp_main[i] <- ifelse("accumulation" %in% vars_split_aic == TRUE, 1, 0)
    BIC_res_temp_main[i] <- ifelse("accumulation" %in% vars_split_bic == TRUE, 1, 0)
    
    # Store if correct interaction in final model
    AIC_res_temp_int[i] <- ifelse("int_accum" %in% vars_split_aic == TRUE, 1, 0)
    BIC_res_temp_int[i] <- ifelse("int_accum" %in% vars_split_bic == TRUE, 1, 0)
    
    # Store if correct main effect and interaction both in final model
    AIC_res_temp_mainint[i] <- ifelse("accumulation" %in% vars_split_aic == TRUE & "int_accum" %in% vars_split_aic == TRUE, 1, 0)
    BIC_res_temp_mainint[i] <- ifelse("accumulation" %in% vars_split_bic == TRUE & "int_accum" %in% vars_split_bic == TRUE, 1, 0)
    ...
    CV_1SE_res_temp_main[i] <- ifelse("accumulation" %in% cv_1SE_covars == TRUE, 1, 0)
    CV_1SE_res_temp_int[i] <- ifelse("int_accum" %in% cv_1SE_covars == TRUE, 1, 0)
    CV_1SE_res_temp_mainint[i] <- ifelse("accumulation" %in% cv_1SE_covars == TRUE & "int_accum" %in% cv_1SE_covars == TRUE, 1, 0)
    ...
    # Store if main effect, interaction, both, and both with additional variables in final model
    CV_minMSE_res_temp_main[i] <- ifelse("accumulation" %in% cv_minMSE_covars == TRUE, 1, 0)
    CV_minMSE_res_temp_int[i] <- ifelse("int_accum" %in% cv_minMSE_covars == TRUE, 1, 0)
    CV_minMSE_res_temp_mainint[i] <- ifelse("accumulation" %in% cv_minMSE_covars == TRUE & "int_accum" %in% cv_minMSE_covars == TRUE, 1, 0)
...
write.csv(results, file = "simulationResults_accum_moderateInt_noChange.csv", row.names = FALSE)
