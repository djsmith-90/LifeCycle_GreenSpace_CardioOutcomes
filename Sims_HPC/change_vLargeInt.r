...
### This script is for: BMI caused by change from time 2 to time 3 and interaction with this and SEP interaction size = very large [4 for binary exposures; 0.04 for continuous exposures])
...
lasso_sim <- function(n_sims = 1000, sampleSize = 1000, Exposure = "Binary", Centered = "No", Collinear = "Low",
  ...
  for (i in 1:n_sims) {
	...
	if (Exposure == "Binary") {
		bmi <- 25 + (-4 * high_sep) + (-2 * green_inc23) + (4 * high_sep * green_inc23) + rnorm(n, 0, 3)
	}
	
	if (Exposure == "Cont") {
		bmi <- 25 + (-4 * high_sep) + (-0.02 * green_ch23) + (0.04 * high_sep * green_ch23) + rnorm(n, 0, 3)
	}
	if (Output == TRUE) {
...
write.csv(results, file = "simulationResults_change_vLargeInt.csv", row.names = FALSE)
