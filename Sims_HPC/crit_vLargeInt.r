...
### This script is for: BMI caused by most recent critical period and interaction with this and SEP (interaction size = very large [4 for binary exposures; 0.04 for continuous exposures])
...
lasso_sim <- function(n_sims = 1000, sampleSize = 1000, Exposure = "Binary", Centered = "No", Collinear = "Low",
  ...
  for (i in 1:n_sims) {
	...
	if (Exposure == "Binary") {
		bmi <- 25 + (-4 * high_sep) + (-2 * green3) + (4 * high_sep * green3) + rnorm(n, 0, 3)
	}
	
	if (Exposure == "Cont") {
		bmi <- 28 + (-4 * high_sep) + (-0.02 * green3) + (0.04 * high_sep * green3) + rnorm(n, 0, 3)
	}
	if (Output == TRUE) {
...
write.csv(results, file = "simulationResults_crit_vLargeInt.csv", row.names = FALSE)
