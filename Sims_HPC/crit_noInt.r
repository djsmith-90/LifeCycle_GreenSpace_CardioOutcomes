...
### This script is for: BMI caused by first critical period and interaction with this and SEP (interaction size = 0)
...
lasso_sim <- function(n_sims = 1000, sampleSize = 1000, Exposure = "Binary", Centered = "No", Collinear = "Low",
  ...
  for (i in 1:n_sims) {
	...
	if (Exposure == "Binary") {
		bmi <- 25 + (-4 * high_sep) + (-2 * green1) + (0 * high_sep * green1) + rnorm(n, 0, 3)
	}
	
	if (Exposure == "Cont") {
		bmi <- 28 + (-4 * high_sep) + (-0.02 * green1) + (0 * high_sep * green1) + rnorm(n, 0, 3)
	}
	if (Output == TRUE) {
