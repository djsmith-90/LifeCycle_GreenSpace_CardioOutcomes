...
## Will start by setting the number of simulations per combination of parameters (1,000), the target hypotheses we simulated are the true model, and set up a data frame to store the results in
n_sims <- 100
set.seed(9876)
...
results$LR_Nworked <- NA
results$LR_Ncorrect <- NA
results$LR_propcorrect <- NA
results$LR_crit1correct <- NA
results$LR_int1correct <- NA
results$LR_crit1int1correct <- NA
results$LR_crit1int1extra <- NA
results$CV_propcorrect <- NA
results$CV_crit1correct <- NA
results$CV_int1correct <- NA
results$CV_crit1int1correct <- NA
results$CV_crit1int1extra <- NA
results
...
LR_res_temp_crit1
LR_res_temp_int1
LR_res_temp_crit1int1
LR_res_temp_crit1int1extra

CV_res_temp
CV_res_temp_crit1
CV_res_temp_int1
CV_res_temp_crit1int1
CV_res_temp_crit1int1extra

# Store the summaries of these results in the main results table
k <- 1
results$LR_Nworked[k] <- sum(!is.na(LR_res_temp))
results$LR_Ncorrect[k] <- sum(LR_res_temp, na.rm = TRUE)
results$LR_propcorrect[k] <- round(results$LR_Ncorrect[k] / results$LR_Nworked[k] * 100, 2)
results$LR_crit1correct[k] <- round(sum(LR_res_temp_crit1, na.rm = TRUE) / results$LR_Nworked[k] * 100, 2)
results$LR_int1correct[k] <- round(sum(LR_res_temp_int1, na.rm = TRUE) / results$LR_Nworked[k] * 100, 2)
results$LR_crit1int1correct[k] <- round(sum(LR_res_temp_crit1int1, na.rm = TRUE) / results$LR_Nworked[k] * 100, 2)
results$LR_crit1int1extra[k] <- round(sum(LR_res_temp_crit1int1extra, na.rm = TRUE) / results$LR_Nworked[k] * 100, 2)

results$CV_propcorrect[k] <- round(sum(CV_res_temp) / n_sims * 100, 2)
results$CV_crit1correct[k] <- round(sum(CV_res_temp_crit1) / n_sims * 100, 2)
results$CV_int1correct[k] <- round(sum(CV_res_temp_int1) / n_sims * 100, 2)
results$CV_crit1int1correct[k] <- round(sum(CV_res_temp_crit1int1) / n_sims * 100, 2)
results$CV_crit1int1extra[k] <- round(sum(CV_res_temp_crit1int1extra) / n_sims * 100, 2)
...
### Now that I've got the script working, I'm tempted to gut it and combine it into a single function, so that everything is much more flexible... But that's a problem for future me!
